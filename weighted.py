...
def dijkstra_path(G, source, target, weight='weight'):
    ...
    (length, path) = single_source_dijkstra(G, source, target=target,
                                            weight=weight)
    try:
        #print length[target],"PPPPPPPPPPPPPPPPPPPPPPP",path[target]
...
def _dijkstra(G, source, get_weight, pred=None, paths=None, cutoff=None,
    ...
    if paths is not None:
        dist,paths = check_link_reliability_constraints(G, dist, paths)


        #print "RETURN",dist, paths
        return (dist, paths)
    if pred is not None:
        return (pred, dist)
    return dist
...
def check_link_reliability_constraints(wsn, dist, paths):
    required_reliability = 50.0
    dist_r = {}
    paths_r = {}
    for k in paths:
        ...
        for idx,n in enumerate(nodes):
            #print "k,nodes", k,nodes
            if idx == 0:
                first_node = n
                if k in dist:
                    dist_r.update({k: dist[k]})
            elif idx > 0:
                link_list.append(1 - wsn[nodes[idx-1]][nodes[idx]]['plr'] / 100.0)
        path_reliability = 0
        if len(link_list) > 1:
        ...
        if path_reliability <= 1 - required_reliability / 100:
            if k in dist:
                dist_r.update({k:dist[k]})
            paths_r.update({k:list(nodes)})
    return dist_r,paths_r
...
def bidirectional_dijkstra(G, source, target, weight='weight'):
    ...
    while fringe[0] and fringe[1]:
        ...
        for w in neighs[dir](v):
            ...
            if(dir == 0):  # forward
                if G.is_multigraph():
                    config.dijkstra_operations += len(G[v][w].items())
                ...
                if G.is_multigraph():
		    config.dijkstra_operations += len(G[w][v].items())
                    minweight = min((dd.get(weight, 1)
